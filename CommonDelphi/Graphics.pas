...
//xarka
    DefDlgFontName : String;
    DefLowFontQuality : Integer;
//
...
{     if lfStrikeOut = 1 then  }
      if lfQuality <> DEFAULT_QUALITY then
//
...
//xarka
{
          ...
          if (AnsiCompareText(Font.name,'MS Sans Serif') = 0) or
             (StringStartsWith('MS SHELL DLG',Font.name)) or
             (AnsiCompareText(Font.name,'Default') = 0)
           then
            StrPCopy(lfFaceName, DefDlgFontName)
           else
            StrPCopy(lfFaceName, Font.Name);
//
...
{         lfQuality := DEFAULT_QUALITY;  }
          if (fsStrikeOut in Font.Style)
           then lfQuality:=DefLowFontQuality
           else lfQuality:=DEFAULT_QUALITY;
//
...
begin
 if (Win32MajorVersion>=5)
  then DefLowFontQuality := NONANTIALIASED_QUALITY
  else DefLowFontQuality := DRAFT_QUALITY;

 if (Win32MajorVersion>=6)
   then DefDlgFontName:='Segoe UI'

 else if (Win32MajorVersion=5)
  then DefDlgFontName:='MS Shell Dlg 2'

 else DefDlgFontName:='MS Shell Dlg';
end;
//
